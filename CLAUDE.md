...
Browser (React SPA)
  ...
  ├── Supabase Edge Function          — proxy Gemini call (ซ่อน API key)
  │   └── supabase/functions/parse-exam/index.ts
  └── SCRIPT_URL (Google Apps Script) — สร้าง Google Form จริงๆ
```

## Key files

- `src/App.tsx` — ทั้งแอปอยู่ในไฟล์เดียว (~1,020 บรรทัด)
- `supabase/functions/parse-exam/index.ts` — Edge Function proxy สำหรับ Gemini
- `vite.config.ts` — Vite config
- `.env` (ไม่อยู่ใน repo) — ต้องมี VITE_SUPABASE_URL, VITE_SUPABASE_KEY, VITE_SCRIPT_URL

## Deploy checklist

1. Set Supabase secret: `supabase secrets set GEMINI_KEY=<your-key>`
2. Deploy Edge Function: `supabase functions deploy parse-exam`
